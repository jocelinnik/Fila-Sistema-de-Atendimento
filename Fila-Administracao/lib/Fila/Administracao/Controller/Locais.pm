...
sub index : Path('') : Args(0) {
  my ( $self, $c ) = @_;
  $c->stash->{locais} =
      $c->model('DB::Local')->search( {}, { order_by => 'nome' } );
}
...
sub reabrir : Chained('preload') : PathPart : Args(0) {
  my ( $self, $c ) = @_;
  $c->stash->{local}->update( { vt_fim => 'Infinity' } );
  $c->res->redirect( $c->uri_for('/locais') );
}
...
sub ver : Chained('preload') : PathPart('') : Args(0) {
  ...
  $c->stash->{guiches} =
      $c->stash->{local}
      ->guiches->search( {}, { order_by => 'identificador' } );

  $c->stash->{configuracoes} =
      $c->stash->{local}->configuracoes_categoria->search(
    {},
    {
      prefetch => 'categoria',
      order_by => [ 'me.vt_ini', 'categoria.nome' ]
    }
      );
  ...
  $c->stash->{conf_perguntas} =
      $c->stash->{local}->configuracoes_perguntas->search( {} );

  $c->stash->{conf_perguntas_praca} =
