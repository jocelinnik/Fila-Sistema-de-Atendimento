...
sub index : Path('') : Args(0) {
  ...
  $c->stash->{funcionarios} = $c->model('DB::Funcionario')->search(
    {},
    {
      ...
      'select' => [
        'me.id_funcionario', 'me.nome',
        'me.jid',            'local_atual.vt_fim',
        'gerente_atual.vt_fim'
      ],
...
sub preload : Chained : PathPart('funcionarios') : CaptureArgs(1) {
  ...
  $c->stash->{funcionario} =
      $c->model('DB::Funcionario')->find( { id_funcionario => $id } )
      or die 'Funcionario nao encontrado';
}
...
sub ver : Chained('preload') : PathPart('') : Args(0) {
  ...
  $c->stash->{locais}   = $c->stash->{funcionario}->locais->search(
    {},
    {
      prefetch => 'local',
      order_by => 'me.vt_ini'
  ...
  $c->stash->{gerentes} = $c->stash->{funcionario}->gerentes->search(
    {},
    {
      prefetch => 'local',
      order_by => 'me.vt_ini'
  ...
  unless ( $c->req->param('submitted') ) {
    $c->req->param( $_, $c->stash->{funcionario}->$_() ) for qw(nome jid);
  }
}
...
sub salvar : Chained('preload') : PathPart('salvar') : Args(0) {
  ...
  unless ( $c->req->param('submitted') ) {
    $c->res->redirect(
      $c->uri_for(
        '/funcionarios/' . $c->stash->{funcionario}->id_funcionario
      )
    );
  }
  else {
...
sub associar_gerente : Chained('preload') : PathPart('gerente/associar') :
    Args(0) {
  my ( $self, $c ) = @_;
...
# primeiro testa para ver se o funcionario mencionado está ligado a uma mesa.
 ...
 # desassocia o gerente atual do local para associar o que ele está pedindo
    my $local =
        $c->model('DB::Local')
        ->find( { id_local => $c->req->param('id_local') },
      { join => 'gerentes' } )
        ->gerentes->search( { 'gerentes.vt_fim' => 'Infinity' } )
