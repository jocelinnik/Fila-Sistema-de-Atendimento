...
sub preload : Chained('/locais/preload') : PathPart('conf_perguntas') :
    CaptureArgs(1) {
  my ( $self, $c ) = @_;
}

sub criar : Chained('/locais/preload') : PathPart('conf_perguntas/criar') :
    Args(0) {
  my ( $self, $c ) = @_;
  if ( $c->req->param('submitted') ) {
    ...
    $c->stash->{local}
        ->configuracoes_perguntas->search( { vt_fim => 'Infinity' } )
