...
sub reabrir : Chained('preload') : PathPart : Args(0) {
  my ( $self, $c ) = @_;
  my $guiche = $c->stash->{guiche}->update( { vt_fim => 'Infinity' } );
  $c->res->redirect(
    $c->uri_for( '/locais/' . $c->stash->{local}->id_local ) );
}
...
sub encerrar_categoria : Chained('preload') : PathPart : Args(1) {
  ...
  $c->stash->{guiche}
      ->categorias_atuais->find( { id_categoria => $id_categoria } )
      ->update( { vt_fim => DateTime->now( time_zone => 'local' ) } );
  $c->res->redirect(
    $c->uri_for(
            '/locais/'
          . $c->stash->{local}->id_local
          . '/guiche/'
          . $c->stash->{guiche}->id_guiche
    )
  );
}
...
sub criar : Chained('/locais/preload') : PathPart('guiche/criar') : Args(0) {
  ...
  if ( $c->req->param('submitted') ) {
    ...
    unless ($estado_fechado) {
      $c->stash->{error} =
          'Ocorreu um erro de configuracao do sistema, estado "fechado" de guiche nao encontrado.';
    }
    else {
