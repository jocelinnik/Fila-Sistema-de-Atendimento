...
sub auto : Private {
  ...
  my $funcionario = $c->model('DB::Funcionario')->search(
    ...
    { prefetch => { 'locais' => { 'local' => { 'estados' => 'estado' } } } }
  )->first();
...
sub listar_guiches : WSDLPort('GestaoGuiche') : DBICTransaction('DB') : MI {
  ...
  while ( my $guiche = $guiches->next ) {
    push @$lista_guiches,
        {
      ( map { $_ => $guiche->$_() } qw/id_local id_guiche identificador/ ),
      estado => $guiche->estados->first->estado->nome
        };
  }

  $c->stash->{soap}
      ->compile_return( { lista_guiches => { guiche => $lista_guiches } } );
}

sub listar_categorias : WSDLPort('GestaoGuiche') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  my $now = $c->stash->{now};
  ...
  while ( my $categoria = $categorias->next ) {
    ...
    push @$lista_categorias,
        { ( map { $_ => $ca->$_() } qw/id_categoria codigo nome/ ) };
  }
