...
sub auto : Private {
  ...
  my $funcionario = $c->model('DB::Funcionario')->search(
    ...
    { prefetch => { 'atendentes' => { 'guiche' => 'local' } } }
  )->first();
...
sub refresh_atendente : Private {
  ...
  if ($atendente) {
    ...
    $c->model('SOAP')
        ->transport->addrs(
      [ $atendente->funcionario->jid . '/cb/render/atendente' ] );
    $c->model('SOAP::CB::Atendente')->render_atendente( {%$guiche} );

    $c->model('SOAP')->transport->addrs(
      [
        $c->stash->{gerente}->funcionario->jid . '/cb/render/guiche_gerente'
      ]
    );
...
sub status_guiche : WSDLPort('GestaoAtendente') : DBICTransaction('DB') : MI {
  ...
  my $guiches = $c->stash->{local}->guiches->search(
    { 'me.id_guiche' => $c->stash->{guiche}->id_guiche },
    {
      ...
      'select' => [
        'me.id_guiche',               'me.identificador',
        'funcionario.id_funcionario', 'funcionario.nome',
        'funcionario.jid',            'estado.nome',
        'estado_atual.vt_ini',        'categoria.codigo',
        'senha.codigo',               'estado_2.nome',
        'estado_atual_2.vt_ini',      'pausa_atual.motivo',
        'agendamento.nome',           'agendamento.tipopessoa',
        'agendamento.cnpjf',          'agendamento.email',
        'me.timeout_chamando',        'me.timeout_concluido',
        'atendimento.id_atendimento', 'me.pular_opiniometro'
      ],
      'as' => [
        'id_guiche',                'identificador',
        'id_funcionario',           'funcionario',
        'jid',                      'estado',
        'estado_desde',             'codigo_categoria',
        'codigo_senha',             'estado_atendimento',
        'estado_atendimento_desde', 'pausa_motivo',
        'agendamento_nome',         'agendamento_tipopessoa',
        'agendamento_cnpjf',        'agendamento_email',
        'timeout_chamando',         'timeout_concluido',
        'id_atendimento',           'pular_opiniometro'
      ]
    }
  );
  ...
  if ( $estado_guiche->estado->nome eq 'interno' ) {
    my $servicos_internos = $c->stash->{guiche}->servico_atual->search(
      {},
      {
        ...
        'as' => [ 'id_servico', 'id_classe', 'nome', 'informacoes' ]
      },
    );
...
sub registrar_no_show : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  my $estado_no_show =
      $c->model('DB::TipoEstadoAtendimento')->find( { nome => 'no_show' } );
  ...
  unless ( $estado_atendimento
    && $estado_atendimento->estado->nome eq 'chamando' )
  {
    die $c->stash->{soap}->fault(
      {
        code   => 'Server',
        reason => 'Estado invalido',
        detail =>
            'Atendimento precisa estar "chamando" para registrar no-show'
      }
    );
  }

  $atend_guiche->update( { vt_fim => $now } );

  $estado_atendimento->update( { vt_fim => $now } );
  ...
  $estado_guiche->update( { vt_fim => $now } );
...
sub listar_no_show : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  ...
  my $list = $c->stash->{local}->atendimentos_atuais->search(
    ...
    {
      prefetch =>
          [ { 'senha' => 'categoria' }, { 'estado_atual' => 'estado' } ]
    }
  );
  ...
  $c->stash->{soap}->compile_return(
    { lista_atendimentos => { atendimento => $retorno } } );

}

sub atender_no_show : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  my $estado_chamando_at =
      $c->model('DB::TipoEstadoAtendimento')->find( { nome => 'chamando' } );
  ...
  my $estado_chamando_gu =
      $c->model('DB::TipoEstadoGuiche')->find( { nome => 'chamando' } );
  ...
  my $atendimento =
      $c->model('DB::Atendimento')->find( $query->{atendimento} );
  unless ($atendimento) {
    die $c->stash->{soap}->fault(
      {
        code   => 'Server',
  ...
  my $estado_atendimento =
      $atendimento->estado_atual->find( {}, { prefetch => 'estado' } );
  unless ( $estado_atendimento
    && $estado_atendimento->estado->nome eq 'no_show' )
  {
    die $c->stash->{soap}->fault(
      {
        code   => 'Server',
        reason => 'Estado invalido',
        detail =>
            'Atendimento precisa estar "no_show" para iniciar atendimento'
      }
    );
  }
...
sub registrar_pendente : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  my $estado_pendente =
      $c->model('DB::TipoEstadoAtendimento')->find( { nome => 'pendente' } );
  ...
  unless ( $estado_atendimento
    && $estado_atendimento->estado->nome eq 'atendimento' )
  {
    die $c->stash->{soap}->fault(
      {
        code   => 'Server',
        reason => 'Estado invalido',
        detail =>
            'Atendimento precisa estar "atendimento" para registrar pendente'
      }
    );
  }
...
sub listar_pendente : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
...
sub atender_pendente : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  unless ( $estado_atendimento
    && $estado_atendimento->estado->nome eq 'pendente' )
  {
    die $c->stash->{soap}->fault(
      {
        code   => 'Server',
        reason => 'Estado invalido',
        detail =>
            'Atendimento precisa estar "pendente" para iniciar atendimento'
      }
    );
  }
...
sub iniciar_atendimento : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  my $estado_atendendo_at =
      $c->model('DB::TipoEstadoAtendimento')
      ->find( { nome => 'atendimento' } );
  ...
  my $estado_atendendo_gu =
      $c->model('DB::TipoEstadoGuiche')->find( { nome => 'atendimento' } );
  ...
  my $estado_atendimento =
      $atendimento->atendimento->estado_atual->find( {},
    { prefetch => 'estado' } );
  unless ( $estado_atendimento
    && $estado_atendimento->estado->nome eq 'chamando' )
  {
    die $c->stash->{soap}->fault(
      {
        ...
        reason => 'Estado invalido',
        detail =>
            'Atendimento precisa estar "chamando" para iniciar atendimento'
      }
    );
  }
...
sub devolver_senha : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        reason => 'Estado invalido',
        detail =>
            'Não conseguiu encontrar estado anterior para devolver o atendimento'
      }
    );
  }

  $estado_anterior->update( { vt_fim => 'Infinity' } );
...
sub concluir_atendimento : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        reason => 'Estado invalido',
        detail =>
            'Guiche precisa estar "atendimento" para concluir atendimento'
      }
    );
  }

  my $estado_avaliacao =
      $c->model('DB::TipoEstadoAtendimento')->find( { nome => 'avaliacao' } );
  ...
  my $estado_guiche_avaliacao =
      $c->model('DB::TipoEstadoGuiche')->find( { nome => 'avaliacao' } );
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        reason => 'Estado invalido',
        detail =>
            'Atendimento precisa estar "atendimento" para concluir atendimento'
      }
    );
  }
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        detail =>
            'Nao e possivel concluir atendimento com servicos em andamento.'
      }
    );
  }
  ...
  if ( $guiche && $guiche->pular_opiniometro == 0 ) {
    ...
    $c->model('SOAP')
        ->transport->addrs(
      [ $c->stash->{guiche}->jid_opiniometro . '/callback' ] );
    ...
    #encerra atendimento
    my $estado_atual =
        $atendimento->atendimento->estado_atual->search( {},
      ...
      die $c->stash->{soap}->fault(
        {
          ...
          detail =>
              'O atendimento precisa estar em "avaliacao" para ser encerrado'
        }
      );
    }
    ...
    $atendimento->atendimento->guiche_atual->first->update(
      { vt_fim => $now } );
    ...
    $c->model('SOAP')
        ->transport->addrs( [ $guiche->jid_opiniometro . '/callback' ] );
    $c->model('SOAP::Opiniometro')
...
sub encaminhar_atendimento : WSDLPort('GestaoAtendente') :
  ...
  my $guiche_origem =
      $c->model('DB::Guiche')
      ->find( id_guiche => $c->stash->{guiche}->id_guiche );
  ...
  my $estado_encaminhado =
      $c->model('DB::TipoEstadoAtendimento')
      ->find( { nome => 'encaminhado' } );
  ...
  $atendimento->atendimento->estado_atual->update( { vt_fim => $now } );
...
sub iniciar_pausa : WSDLPort('GestaoAtendente') : DBICTransaction('DB') : MI {
  ...
  unless (
    $estado_guiche
    && ( ( $estado_guiche->estado->nome eq 'concluido' )
      || ( $estado_guiche->estado->nome eq 'disponivel' ) )
      )
  {
    die $c->stash->{soap}->fault(
      {
        ...
        reason => 'Estado invalido',
        detail =>
            'Guiche deve estar "concluido" ou "disponivel" para iniciar uma pausa.'
      }
    );
  }
  ...
  my $estado_pausa =
      $c->model('DB::TipoEstadoGuiche')->find( { nome => 'pausa' } );
  unless ($estado_pausa) {
    die $c->stash->{soap}->fault(
      {
        code   => 'Server',
  ...
  $c->stash->{guiche}->pausas->create(
    {
      ...
      vt_fim => 'Infinity',
      id_funcionario =>
          $c->stash->{guiche}->atendente_atual->first->id_funcionario,
      motivo => ''
    }
  );
...
sub setar_motivo_pausa : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  $pausa_atual->update( { motivo => $pausa_motivo } );

  $c->stash->{refresh_guiche} ||= [];
  push @{ $c->stash->{refresh_guiche} }, $c->stash->{guiche}->id_guiche;
}

sub fechar_servico_interno : WSDLPort('GestaoAtendente') :
    DBICTransaction('DB') : MI {
  my ( $self, $c ) = @_;
...
sub disponivel : WSDLPort('GestaoAtendente') : DBICTransaction('DB') : MI {
  ...
  unless (
    $estado_guiche
    && ( ( $estado_guiche->estado->nome eq 'concluido' )
      || ( $estado_guiche->estado->nome eq 'pausa' ) )
      )
  {
    die $c->stash->{soap}->fault(
      {
        ...
        reason => 'Estado invalido',
        detail =>
            'Guiche deve estar "concluido" ou em "pausa" para voltar a estar disponivel'
      }
    );
  }
  ...
  my $estado_disponivel =
      $c->model('DB::TipoEstadoGuiche')->find( { nome => 'disponivel' } );
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        detail =>
            'Nao pode voltar a estar disponivel enquanto atendimento estiver associado ao guiche.'
      }
    );
  }
...
sub fechar_guiche : WSDLPort('GestaoAtendente') : DBICTransaction('DB') : MI {
  ...
  my $estado_fechado =
      $c->model('DB::TipoEstadoGuiche')->find( { nome => 'fechado' } );
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        reason => 'Atendimento ainda associado.',
        detail =>
            'Nao pode fechar enquanto atendimento estiver associado ao guiche.'
      }
    );
  }
  ...
  my $atendente = $c->stash->{guiche}->atendentes->find(
    {
      vt_ini => { '<=', $now },
      vt_fim => { '>',  $now }
    }
...
sub iniciar_servico_interno : WSDLPort('GestaoAtendente') :
    DBICTransaction('DB') : MI {
  my ( $self, $c, $query ) = @_;
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        reason => 'Estado invalido',
        detail =>
            'Guiche deve estar "concluido" ou "disponivel" para iniciar um serviço interno.'
      }
    );
  }
  ...
  my $estado_interno =
      $c->model('DB::TipoEstadoGuiche')->find( { nome => 'interno' } );
...
sub listar_servicos : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c ) = @_;
  ...
  while ( my $servico = $servicos->next ) {
    push @$lista_servicos,
        {
      ( map { $_ => $servico->$_() } qw/id_servico id_classe nome/ ),
...
sub listar_servicos_atendimento : WSDLPort('GestaoAtendente') :
    DBICTransaction('DB') : MI {
  my ( $self, $c ) = @_;
...
sub setar_info_interno : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  $servico_atual->update( { informacoes => $informacoes } );

  $c->stash->{refresh_guiche} ||= [];
  push @{ $c->stash->{refresh_guiche} }, $c->stash->{guiche}->id_guiche;
}

sub iniciar_servico_atendimento : WSDLPort('GestaoAtendente') :
    DBICTransaction('DB') : MI {
  my ( $self, $c, $query ) = @_;
  ...
  unless ( $estado_guiche
    && ( $estado_guiche->estado->nome eq 'atendimento' ) )
  {
    die $c->stash->{soap}->fault(
      {
        code   => 'Server',
        reason => 'Estado invalido',
        detail =>
            'Guiche deve estar "atendimento" para iniciar um serviço em atendimento.'
      }
    );
  }
...
sub fechar_servico_atendimento : WSDLPort('GestaoAtendente') :
    DBICTransaction('DB') : MI {
  my ( $self, $c, $query ) = @_;
  ...
  #pegando servico
  my $servico_atual =
      $atendimento->servico_atual->search( { id_servico => $id_servico } );

  #encerra objeto servico
  $servico_atual->update( { vt_fim => $now } );

  $c->stash->{refresh_guiche} ||= [];
  push @{ $c->stash->{refresh_guiche} }, $c->stash->{guiche}->id_guiche;
}

sub setar_info_atendimento : WSDLPort('GestaoAtendente') :
    DBICTransaction('DB') : MI {
  my ( $self, $c, $query ) = @_;
...
sub retornar_pausa : WSDLPort('GestaoAtendente') : DBICTransaction('DB')
    : MI {
  my ( $self, $c ) = @_;
...
sub mudar_senha : WSDLPort('GestaoAtendente') : DBICTransaction('DB') : MI {
  ...
  $c->stash->{soap}->compile_return(
    { guiche => { senha => 'Senha alterada com sucesso' } } );
