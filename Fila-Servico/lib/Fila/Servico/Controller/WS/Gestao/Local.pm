...
sub refresh_painel : Private {
  ...
  while ( my $atendimento = $atendimentos->next ) {
    ...
    push @$ret,
        {
      senha => sprintf( '%s%04d',
        $atendimento->senha->categoria->codigo,
  ...
  $c->model('SOAP')
      ->transport->addrs( [ $c->stash->{local}->jid_painel . '/callback' ] );
  $c->model('SOAP::Painel')
      ->senhas_chamando( { senhas_chamando => { senha => $ret } } );
  ...
  $storage->txn_do(
    sub {
      ...
      $c->model('SOAP')->transport->addrs(
        [
          $c->stash->{local}->gerente_atual->first->funcionario->jid
              . '/cb/render/gerente'
        ]
      );
      $c->model('SOAP::CB::Gerente')
          ->render_gerente( { %$local, %$guiches, %$encaminhamentos } );
...
sub dados_local : WSDLPort('GestaoLocal') : DBICTransaction('DB') : MI {
  ...
  $c->stash->{soap}->compile_return(
    {
      local => {
        ( map { $_ => $c->stash->{local}->$_() } qw/ id_local nome / ),
        (
          map {
            ( $c->stash->{local}->$_ && $c->stash->{local}->$_->is_infinite )
                ? ()
                : (
...
sub abrir_local : WSDLPort('GestaoLocal') : DBICTransaction('DB') : MI {
  ...
  my $estado_aberto =
      $c->model('DB::TipoEstadoLocal')->find( { nome => 'aberto' } );
  ...
  $c->model('SOAP')->transport->connection( $c->engine->connection($c) );
  $c->model('SOAP')
      ->transport->addrs( [ $c->stash->{local}->jid_senhas . '/callback' ] );
  $c->model('SOAP::Senha')
      ->local_aberto( { refresh_request => 'non-empty' } );
...
sub encerrar_senhas : WSDLPort('GestaoLocal') : DBICTransaction('DB') : MI {
  ...
  my $estado_senc =
      $c->model('DB::TipoEstadoLocal')
      ->find( { nome => 'senhas_encerradas' } );
  ...
  $c->model('SOAP')
      ->transport->addrs( [ $c->stash->{local}->jid_senhas . '/callback' ] );
  $c->model('SOAP::Senha')
      ->senhas_encerradas( { refresh_request => 'non-empty' } );
  $c->stash->{refresh_gerente} = 1;
...
sub status_local : WSDLPort('GestaoLocal') : DBICTransaction('DB') : MI {
  ...
  my $busca_gerente =
      $c->model('DB::Funcionario')->find( { id_funcionario => $id_gerente } );
  ...
  my $lista = $c->stash->{local}->configuracoes_categoria_atual->search(
    { 'estado.nome' => 'espera' },
    {
      join => {
        categoria => {
          atendimentos_atuais =>
              { atendimento => { estado_atual => 'estado' } }
        }
      },
      select => [
        'me.id_categoria',
      ...
      as => [
        'id_categoria',        'codigo',
        'espera_mais_antiga',  'quantidade_espera',
        'limite_tempo_espera', 'limite_pessoas_espera'
      ],
      group_by => [
        'me.id_categoria',        'categoria.codigo',
  ...
  while ( my $categoria = $lista->next ) {
    ...
    $espera =
      ...
      DateTime::Format::Pg->parse_datetime($espera)->set_time_zone('local') )
        if $espera;

    if ( $espera && $categoria->get_column('limite_tempo_espera') ) {
      my $base = DateTime::Format::XSD->parse_datetime($espera);
      my $dif =
          DateTime->now( time_zone => 'local' )->subtract_datetime($base);
      my $lim =
          DateTime::Duration->new(
        seconds => $categoria->get_column('limite_tempo_espera') );
...

  {
    ...
    my $lista = $c->stash->{local}->configuracoes_categoria->search(
      ...
      },
      {
        join => {
          categoria =>
              { atendimentos => { atendimento => { estados => 'estado' } } }
        },
        select => [
          'me.id_categoria',
          'categoria.codigo',
    ...
    while ( my $categoria_estado = $lista->next ) {
      ...
      $categorias{$id_categoria}{ "tempo_medio_" . $estado } =
          $categoria_estado->get_column('media');
    }
  }

  {
    ...
    while ( my $estado = $lista->next ) {
      ...
      $agregado->{ "tempo_medio_" . $strestado } =
          $estado->get_column('media');
    }
  }
  ...
  if ($ultimo_aberto) {
    ...
    my @e_g = $c->model('DB::GuicheEncaminhamento')->search(
      ...
      {
        select => [
          { count => 'me.id_atendimento' },
          \"CASE WHEN vt_fim > NOW() THEN 1 ELSE 0 END"
        ],
        as => [ 'encaminhamentos', 'esperando' ],
        group_by => \"CASE WHEN vt_fim > NOW() THEN 1 ELSE 0 END"
      }
    )->all;
    ...
    $total_enc_abertos = reduce { $a + $b->get_column('encaminhamentos') } 0,
        @esp;
  }
...
sub fechar_local_force : WSDLPort('GestaoLocal') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  my $estado_fechado =
      $c->model('DB::TipoEstadoLocal')->find( { nome => 'fechado' } );
...
sub status_guiches : WSDLPort('GestaoLocal') : DBICTransaction('DB') : MI {
  ...
  my $guiches = $c->stash->{local}->guiches->search(
    ...
    {
      'join' => [
        { 'estado_atual' => { 'estado' => 'limites_atuais' } },
        'pausa_atual',
      ...
      'select' => [
        ...
        'me.pular_opiniometro',       'funcionario.id_funcionario',
        'funcionario.nome',           'funcionario.jid',
        'estado.nome',                'estado_atual.vt_ini',
        'categoria.codigo',           'senha.codigo',
        'estado_2.nome',              'estado_atual_2.vt_ini',
        'atendimento.id_atendimento', 'pausa_atual.motivo',
        'servico.nome',               'limites_atuais.segundos',
      ],
      'as' => [
        'id_guiche',          'identificador',
        'pular_opiniometro',  'id_funcionario',
        'funcionario',        'jid',
        'estado',             'estado_desde',
        'codigo_categoria',   'codigo_senha',
        'estado_atendimento', 'estado_atendimento_desde',
        'id_atendimento',     'pausa_motivo',
        'nome_servico',       'limite'
      ],
  ...
  while ( my $guiche = $guiches->next ) {
    ...
    my %dates = (
      map {
        $guiche->get_column($_)
            ? (
          $_ => DateTime::Format::XSD->format_datetime(
            DateTime::Format::Pg->parse_datetime( $guiche->get_column($_) )
                ->set_time_zone('local')
          )
            )
            : ()
    ...
    if ( $dates{estado_desde} && $guiche->get_column('limite') ) {
      my $base =
          DateTime::Format::XSD->parse_datetime( $dates{estado_desde} );
      my $dif =
          DateTime->now( time_zone => 'local' )->subtract_datetime($base);
      my $sec = DateTime::Duration->new(
        'seconds' => $guiche->get_column('limite') );
  ...
  $c->stash->{soap}
      ->compile_return( { lista_guiches => { guiche => $ret } } );
...
sub escalonar_senha : WSDLPort('GestaoLocal') : DBICTransaction('DB') : MI {
  ...
  while ( my $guiche = $guiches->next ) {
      ...
      if ($enc) {
        ...
        $atendimento = $c->stash->{local}->atendimentos_atuais->find(
          ...
          },
          {
            ...
            'order_by' =>
                '((now() - estado_atual.vt_ini) * configuracoes_atuais.prioridade) DESC'
          }
        );
      }
    }
    ...
    if ($atendimento) {
      my $outros_guiches =
          $atendimento->guiches->search( { 'me.vt_fim' => { '>', $now } } );
      while ( my $outro_guiche = $outros_guiches->next ) {
        $outro_guiche->update( { vt_fim => $now } );
      }

      my $outros_estados_guiche =
          $guiche->estados->search( { 'me.vt_fim' => { '>', $now } } );
      while ( my $outro_estado = $outros_estados_guiche->next ) {
        $outro_estado->update( { vt_fim => $now } );
      }
      ...
      my $estados =
          $atendimento->estados->search( { 'me.vt_fim' => { '>', $now } } );

      while ( my $outro_estado = $estados->next ) {
        $estados->update( { vt_fim => $now } );
      }
...
sub dados_funcionario : WSDLPort('GestaoLocal') : DBICTransaction('DB') : MI {
  ...
  my $funcionario = $c->model('DB::Funcionario')->find( { jid => $jid } );
  ...
  $c->stash->{soap}
      ->compile_return( { funcionario => { nome => $funcionario->nome } } );

}

sub encerrar_atendimento : WSDLPort('GestaoLocal') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  my $estado_atual =
      $atendimento->estado_atual->search( {}, { prefetch => 'estado' } )
      ->first;
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        detail =>
            'Nenhum guichê encontrado com estado concluido ou disponivel.'
      }
    );
  }
...
sub concluir_atendimento : WSDLPort('GestaoLocal') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
...
sub pular_opiniometro : WSDLPort('GestaoLocal') : DBICTransaction('DB') : MI {
  ...
  $guiche->update( { pular_opiniometro => $valor } );

  $c->stash->{refresh_gerente} = 1;
}

sub listar_encaminhamentos : WSDLPort('GestaoLocal') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  if ($ultimo_aberto) {
    ...
    my $enc_guiches = $c->model('DB::GuicheEncaminhamento')->search(
      { 'me.vt_ini' => { '>=', $vt_ini } },
      {
        join   => [ 'guiche', 'guiche_origem' ],
        select => [
          'me.vt_ini',                   'guiche.identificador',
          'guiche_origem.identificador', 'me.id_atendimento',
          'me.informacoes',              'me.vt_fim'
        ],
        as => [
          'vt_ini',           'id_guiche',
          'id_guiche_origem', 'id_atendimento',
          'informacoes',      'vt_fim'
        ],
    ...
    my $enc_categorias = $c->model('DB::CategoriaEncaminhamento')->search(
      { 'me.vt_ini' => { '>=', $vt_ini } },
      {
        join   => [ 'categoria', 'guiche_origem' ],
        select => [
          'me.vt_ini',                   'categoria.codigo',
          'guiche_origem.identificador', 'me.id_atendimento',
          'me.informacoes',              'me.vt_fim'
        ],
        as => [
    ...
    while ( $eg || $ec ) {
      ...
      push @$ret, {
        ...
        (
          map {
                  ( $encaminhamento->$_ && $encaminhamento->$_->is_infinite )
                ? ()
                : (
              $_ => DateTime::Format::XSD->format_datetime(
                $encaminhamento->$_->set_time_zone('local')
              )
                )
          } qw/ vt_ini vt_fim /
        ),
    ...
    die $c->stash->{soap}->fault(
      {
        ...
        detail =>
            'Atendente nao pode ser gerente se estiver ligado a uma mesa.'
      }
    );
  }
  $c->stash->{gerente}
...
sub listar_funcionarios : WSDLPort('GestaoLocal') : DBICTransaction('DB')
    : MI {
  my ( $self, $c, $query ) = @_;
  ...
  while ( my $funcionario = $funcionarios->next ) {
    ...
    unless (
      ...
      || ( $funcionario->id_funcionario
        == $c->stash->{gerente}->get_column('id_funcionario') )
        )
    {
