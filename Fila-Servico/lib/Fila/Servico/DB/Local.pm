...
__PACKAGE__->add_columns(
  ...
  jid_senhas      => { data_type => 'varchar', },
  jid_painel      => { data_type => 'varchar', },
  jid_opiniometro => { data_type => 'varchar', }
);
...
__PACKAGE__->has_many(
  'encaminhamentos_categoria',
  'Fila::Servico::DB::CategoriaEncaminhamento',
  { 'foreign.id_local' => 'self.id_local' },
  { join_type          => 'left' }
);

__PACKAGE__->has_many(
  'configuracoes_limite',
...
__PACKAGE__->has_many(
  ...
  'Fila::Servico::DB::ConfiguracaoPerguntas',
  { 'foreign.id_local' => 'self.id_local' }
);
__PACKAGE__->has_many(
