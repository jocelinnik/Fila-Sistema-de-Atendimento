...
sub create_action {
    ...
    && ref $args{attributes}{ActionClass} eq 'ARRAY' )
  {
    ...
    if ($mi) {
      @ActionClasses =
          grep { !$_->isa('Fila::Servico::Action::MI') } @ActionClasses;
...
sub end : Private {
    ...
    && $c->req->header('XMPP_Stanza') eq 'message' )
  {
    ...
    $c->model('SOAP::CB::Error')->render_error(
      {
        Fault => {
    ...
    if ( $c->stash->{refresh_gerente} ) {
      $c->model('SOAP')->transport->connection( $c->engine->connection($c) );
      $c->model('SOAP')
          ->transport->addrs( ['motor@gestao.fila.vhost/ws/scheduler'] );
      $c->model('SOAP::Scheduler')
          ->refresh_gerente(
        { local => { id_local => $c->stash->{local}->id_local } } );
    }
    ...
    if ( $c->stash->{refresh_guiche} ) {
      for my $id_guiche ( @{ $c->stash->{refresh_guiche} } ) {
        $c->model('SOAP')
            ->transport->connection( $c->engine->connection($c) );
        $c->model('SOAP')
            ->transport->addrs( ['motor@gestao.fila.vhost/ws/scheduler'] );
        $c->model('SOAP::Scheduler')
            ->refresh_guiche( { guiche => { id_guiche => $id_guiche } } );
      }
    }
...

{
  ...
  sub new {
      ...
      && ref $args->{attributes}{ActionClass} eq 'ARRAY' )
    {
      ...
      my @others = grep { $_ ne $self } @{ $args->{attributes}{ActionClass} };
      no strict 'refs';
