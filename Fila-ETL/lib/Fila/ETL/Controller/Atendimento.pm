  ...
  $c->model('Federado')->doeach(
    ...
    sub {
      ...
      my $consulta = $local->atendimentos->search(
        {
          -and => [
            { 'me.vt_ini' => { '>'  => $c->stash->{last_vt_base} } },
...

        {
          ...
          select => [
            \'count(distinct me.id_atendimento)',
          ...
          group_by => [
            ...
            'guiche.id_local',            'guiche.vt_ini',
            'guiche.vt_fim',              'guiche.identificador',
            'guiche.jid_opiniometro',     'guiche.pular_opiniometro',
            'atendentes.id_funcionario',  'atendentes.id_guiche',
            'atendentes.vt_ini',          'atendentes.vt_fim',
            'funcionario.id_funcionario', 'funcionario.nome',
            'funcionario.jid',            'funcionario.password',
...
GROUP BY
  ...
  tipo_estado_atendimento.nome
      ...
      while ( my ($datahora) = $sth->fetchrow_array ) {
        ...
        $sth_inner->execute( $datahora, $datahora, $id, $datahora,
          $datahora );
        while ( my $item = $sth_inner->fetchrow_hashref ) {
          my $id_categoria = $item->{id_categoria};
          $counters_cat->{$id_categoria}{ $item->{nome_estado} } =
              $item->{quantidade};
        ...
        foreach my $id_categoria ( keys %{$counters_cat} ) {
          unless ( exists $cat_cache->{$id_categoria} ) {
            $cat_cache->{$id_categoria} =
                $c->model('DB::DCategoria')->get_dimension(
              Fila::Servico::DB::Categoria->new(
                {
          ...
          $c->model('DB::FQuantidadeEstados')->create(
            {
              ...
              (
                map {
                  'quantidade_'
                      . $_ => ( $counters_cat->{$id_categoria}{$_} || 0 )
                    } 'espera',
                'chamando',
                'atendimento',
                'avaliacao'
      ...
      while ( my $item = $sth->fetchrow_hashref ) {
        ...
        unless ( exists $cat_cache->{$id_categoria} ) {
          $cat_cache->{$id_categoria} =
              $c->model('DB::DCategoria')->get_dimension(
            Fila::Servico::DB::Categoria->new(
              { map { $_ => $item->{$_} } qw(id_categoria nome codigo) }
            )
              );
        }
        my $categoria = $cat_cache->{$id_categoria};
