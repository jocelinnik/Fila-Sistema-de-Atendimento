...

{
	...
	my $back_pixbuf =  Gtk2::Gdk::Pixbuf->new_from_file('/usr/share/fila/Fila-Senha/Fila-Senha-Emissor-Gtk2-Cejam.png');
	...
	if ($::hora) { 
           $::hora->modify_font(Gtk2::Pango::FontDescription->from_string("Arial 50"));
           $::hora->modify_fg($::header->state, Gtk2::Gdk::Color->parse("#FFFFFF"));
           $::hora->modify_bg($::header->state, Gtk2::Gdk::Color->parse("#000000"));
	}
        if ($::minuto) { 
           $::minuto->modify_font(Gtk2::Pango::FontDescription->from_string("Arial 50"));
           $::minuto->modify_fg($::header->state, Gtk2::Gdk::Color->parse("#FFFFFF"));
           $::minuto->modify_bg($::header->state, Gtk2::Gdk::Color->parse("#000000"));
	}

	$::horario->modify_font(Gtk2::Pango::FontDescription->from_string("Arial 20"));
	$::horario->modify_fg($::header->state, Gtk2::Gdk::Color->parse("#FFFFFF"));
	$::horario->modify_bg($::header->state, Gtk2::Gdk::Color->parse("#000000"));

	$::header->modify_font(Gtk2::Pango::FontDescription->from_string("Arial 20"));
	$::header->modify_fg($::header->state, Gtk2::Gdk::Color->parse("#FFFFFF"));
	$::header->modify_bg($::header->state, Gtk2::Gdk::Color->parse("#000000"));
	$::header->set_label('AGUARDE');
	$::footer = $::gladexml->get_widget('footer');
	$::footer->modify_font(Gtk2::Pango::FontDescription->from_string("Arial 20"));
	$::footer->modify_fg($::footer->state, Gtk2::Gdk::Color->parse("#FFFFFF"));
	$::footer->modify_bg($::footer->state, Gtk2::Gdk::Color->parse("#000000"));
	$::header->set_label('AGUARDE');
	$::footer->set_label('...');
	...
	if ($::connection->is_connected && defined $::chamar) {
		...
		my $dados = Fila::Senha->model('SOAP::Senha')->solicitar_senha
			({ atendimento => { id_categoria => $::chamar,
					    vt_ini => DateTime::Format::XSD->format_datetime(DateTime::Format::XSD->parse_datetime("$ano-$mes-$dia".'T'."$hora:$minuto:00-02:00")),
			} });
		if ($dados->{Fault}) {
...
sub escalonar_senha {
        if ($::connection->is_connected) {
                my $dados = Fila::Senha->model('SOAP::Senha')->escalonar_senha({
			local => { id_local => 2 }
		});
                if ($dados->{Fault}) {
