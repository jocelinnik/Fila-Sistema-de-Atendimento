...
sub authenticated : Regex('^render/authenticated$') {
  ...
  $c->stash->{'s'} = Digest::MD5::md5_hex(
    $::user_jid . 'senha de controle interno do sistema' );
  $c->forward( $c->view() );
  ...
  my $dados_local =
      $c->model('SOAP::Gestao::Local')->dados_local( { local => {} } );
    ...
    && $dados_local->{Fault}{faultstring} =~ /Permissao Negada/ )
  {
    $c->model('SOAP')
        ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/guiche'] );
    $dados_local =
        $c->model('SOAP::Gestao::Guiche')->dados_local( { local => {} } );
    $mode = 'atendente';
  }
...
sub emissor : Private {
  ...
  my $dados_local =
      $c->model('SOAP::Gestao::Senha')->dados_local( { local => {} } );

  if ( $dados_local->{Fault}
    && $dados_local->{Fault}{faultstring} =~ /Permissao Negada/ )
  {
    $c->stash->{status_local} =
        'Local está fechado. Vai esperar uma notificacao.';
  }
  elsif ( $dados_local->{Fault} ) {
    $c->stash->{status_local} = 'Erro ao obter os dados do local: '
    ...
    && $status_guiche->{Fault}{faultstring} =~ /Permissao Negada/ )
  {
      ...
      && $status_guiche->{guiche}{identificador} ne $::user_guiche )
    {
      ...
      $c->stash->{error_message} =
            'ATENÇÃO: Você está associado à mesa '
          . $status_guiche->{guiche}{identificador}
          . ' mas está agora na mesa '
          . $::user_guiche
          . '. Você precisa fechar a mesa e entrar novamente '
...
sub gerente : Private {
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  my $status_guiches =
      $c->model('SOAP::Gestao::Local')->status_guiches( { local => {} } );
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  my $status_local =
      $c->model('SOAP::Gestao::Local')->status_local( { local => {} } );
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  my $lista_encaminhamentos =
      $c->model('SOAP::Gestao::Local')
