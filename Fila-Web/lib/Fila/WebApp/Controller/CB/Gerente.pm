...
sub abrir_local : WSDLPort('FilaWebGerenteCallback') {
  ...
  my $dados_local =
      $c->model('SOAP::Gestao::Local')->abrir_local( { local => {} } );
...
sub fechar_local : WSDLPort('FilaWebGerenteCallback') {
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  my $fechar_local =
      $c->model('SOAP::Gestao::Local')->fechar_local( { local => {} } );
}
...
sub fechar_local_force : WSDLPort('FilaWebGerenteCallback') {
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  my $fechar_local =
      $c->model('SOAP::Gestao::Local')->fechar_local_force( { local => {} } );
}
...
sub encerrar_senhas : WSDLPort('FilaWebGerenteCallback') {
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  my $encerrar_senhas =
      $c->model('SOAP::Gestao::Local')->encerrar_senhas( { local => {} } );
}
...
#busca no motor pela funcao dados_funcionario para retornar o nome do funcionario
    ...
    $c->model('SOAP')
        ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
    my $dados_funcionario =
        $c->model('SOAP::Gestao::Local')
        ->dados_funcionario( { funcionario => { jid => bare_jid $too } } );
...
sub receber_chat : Local {
  ...
  my $dados_funcionario =
      $c->model('SOAP::Gestao::Local')
      ->dados_funcionario( { funcionario => { jid => $from } } );
...
sub encerrar_atendimento : WSDLPort('FilaWebGerenteCallback') {
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  $c->model('SOAP::Gestao::Local')
      ->encerrar_atendimento( { atendimento => {%params} } );
...
sub devolver_senha : WSDLPort('FilaWebGerenteCallback') {
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  $c->model('SOAP::Gestao::Local')->devolver_senha( { guiche => {%params} } );
...
sub fechar_todos : WSDLPort('FilaWebGerenteCallback') {
  ...
  $c->model('SOAP')
      ->transport->addrs( ['motor@gestao.fila.vhost/ws/gestao/local'] );
  my $fechar_todos =
      $c->model('SOAP::Gestao::Local')->fechar_todos( { guiche => {} } );
}
