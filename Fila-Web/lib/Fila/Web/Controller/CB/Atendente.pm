...
sub fechar_guiche : Local {
  ...
  $c->model('SOAP')
      ->transport->addrs( [ $c->session->{user_jid} . '/cb/atendente' ] );
...
sub listar_guiches_encaminhar : Local {
  ...
  my $req =
      $c->model('SOAP::Atendente')
      ->listar_guiches_encaminhar( { callback_request => { param => [] } } );
}
...
sub mudar_senha : Local {
  ...
  unless ( ( $confsenha eq $novasenha ) || ($confsenha) || ($novasenha) ) {
    $c->stash->{error_message} =
        'Senhas não conferem ou não foram informadas';
    $c->stash->{mudar_senha} =
        'Senhas não conferem ou não foram informadas';
