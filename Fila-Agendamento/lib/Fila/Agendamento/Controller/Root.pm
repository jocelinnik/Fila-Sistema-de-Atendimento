...
sub passo0 : Local Args(0) {
  ...
  if ( $c->req->param('tipopessoa') eq 'F'
    && test_id( 'cpf', $c->req->param('cnpjf') ) )
  {
    $c->session->{cnpjf} = canon_id( 'cpf', $c->req->param('cnpjf') );
  }
  elsif ( $c->req->param('tipopessoa') eq 'J'
    && test_id( 'cnpj', $c->req->param('cnpjf') ) )
  {
  ...
  if (@missing) {
    ...
    $c->res->redirect(
      $c->uri_for( '/' . $c->req->param('id_local') . '/passo1' ) );
  }
}
...
sub passo2 : Chained('preload') : Args(2) {
  ...
  my $senha =
      'A' . sprintf( "%02d", int( $hora * 2 + ( int( $minuto / 30 ) ) ) );
  ...
  $c->model('DB')->schema->txn_do(
    sub {
      my $at = $c->stash->{local}->atendimentos->count( { data => $dt } );
      my $at2 = $c->stash->{local}->atendimentos->search(
        {
      ...
      if ( $at > $Fila::Agendamento::quantidade_por_intervalo ) {
        $c->flash->{error} =
            '<P>Agendamento n&atilde;o realizado, hor&aacute;rio n&atilde;o dispon&iacute;vel.</P>';
        $c->res->redirect(
          $c->uri_for( '/' . $c->stash->{local}->id_local . '/passo1' ) );
      }
      if ($at2) {
        $c->flash->{error} =
...
sub atendimento : Local : Args(0) {
  ...
  $c->stash->{error} = $c->flash->{error};
  $c->stash->{atendimento} =
      $c->model('DB::Atendimento')
      ->find( { id_atendimento => $id_atendimento },
    { prefetch => 'local' } );
  $c->stash->{email} = {
