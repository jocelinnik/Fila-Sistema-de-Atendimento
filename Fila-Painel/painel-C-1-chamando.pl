...
$| = 1;
use constant {
	INPUT_FILE => '/home/fila/painel/senhas_chamando.csv',
		   TEMPO_ROTACAO => 5,
		   TEMPO_RECHAMADA => 90,
		   TEMPO_ULTIMAS => 8,
...
my ($window, $lblsenha, $lblultima, $label_time, $label_clima, $viewport_news, $label_news) = map { $gladexml->get_widget($_) }qw(window1 label_chamando label_ultimas label_time label_clima viewport_news label_news);
...
sub atualizar_noticias {
  $posicao += 6;
  $posicao = $viewport_news->get_hadjustment->lower() if $posicao > $viewport_news->get_hadjustment->upper();
...
sub atualizar_clima {
   ...
   $label_clima->set_label("Temperatura: ".$gw->temp_c.'°C ('.$gw->today('low').'/'.$gw->today('high').'°C)'."\n".
                          $gw->condition." — ".$gw->humidity);
   return 1;
}
...
#
	...
	my @new =
		grep { my $a = $_;
			!(grep { $a->[0] eq $_->[0] && $a->[1] eq $_->[1] } @rotacao) } @arquivo;

	my @old =
		grep { my $a = $_;
			!(grep { $a->[0] eq $_->[0] && $a->[1] eq $_->[1] } @arquivo) } @rotacao;
		...
		foreach my $a (@arquivo) {
			if ($a->[0] eq $rotacao[$k]->[0] && $a->[1] eq $rotacao[$k]->[1]) {
				if ($time - $rotacao[$k][2] > TEMPO_RECHAMADA) {
					...
					unshift (@chamar_atencao, [$rotacao[$k][0], $rotacao[$k][1], $time]);
				} else {
					push (@rot, $rotacao[$k]);
				}
			}
	...
	if (@chamar_atencao) {
		...
		if ($paginas) {
			...
			foreach my $chamada (@now) {
				...
				push (@sons, ('SENHA', split(//, $chamada->[0]),'GUICHE', split(//, $chamada->[1])));
			}
			$lblsenha->set_label($str);
			$to_atencao = undef;
			if (@chamar_atencao) {
				$to_atencao = Glib::Timeout->add(TEMPO_ROTACAO * 1000, \&cycle_chamando);
			} else {
				$to_rotacao = Glib::Timeout->add(TEMPO_ROTACAO * 1000, \&cycle_chamando);
			}
			&alert(@sons);
		}
	} else {
...
sub setup_widgets {
	...
	$window->set_decorated(0);
        $window->move(1025,0);
	$window->modify_bg($window->state, Gtk2::Gdk::Color->parse("#FFFFFF"));
	Gtk2::Window::fullscreen($window);
	$lblsenha->modify_font(Gtk2::Pango::FontDescription->from_string("Courier Bold 72")); $lblsenha->modify_fg($lblsenha->state, Gtk2::Gdk::Color->parse("#FF0000"));
	$lblultima->modify_font(Gtk2::Pango::FontDescription->from_string("Courier Bold 72")); $lblultima->modify_fg($lblultima->state, Gtk2::Gdk::Color->parse("#333333"));
	#$label_clima->modify_font(Gtk2::Pango::FontDescription->from_string("Tahoma 36")); $label_clima->modify_fg($label_clima->state, Gtk2::Gdk::Color->parse("Blue"));
	#$label_time->modify_font(Gtk2::Pango::FontDescription->from_string("Tahoma 36")); $label_time->modify_fg($label_time->state, Gtk2::Gdk::Color->parse("i#195b37"));
	$label_news->modify_font(Gtk2::Pango::FontDescription->from_string("Tahoma 46")); $label_news->modify_fg($label_news->state, Gtk2::Gdk::Color->parse("#195B37"));
	$viewport_news->modify_bg($label_news->state,Gtk2::Gdk::Color->parse("#FFFFFF"));
	$window->show_all();
}
