...
sub check_input_file {
  ...
  my @new =
      grep {
    my $a = $_;
    !( grep { $a->[0] eq $_->[0] } @rotacao )
      } @arquivo;

  push @chamar_atencao, @new;
  push @rotacao,        @new;

  push @chamar_atencao, grep {
           ( ( $time - $_->[2] ) > TEMPO_RECHAMADA )
        && ( $_->[2] = $time )
  } @rotacao;
  ...
  foreach my $categoria (@categorias) {
    delete $ultimas{$categoria}
        if ( ( time - $ultimas{$categoria}[2] ) > TEMPO_TIRAR_ULTIMAS );
  }
...
sub setup_widgets {
  ...
  $lblsenha->modify_font(
    Gtk2::Pango::FontDescription->from_string("Courier New Bold 72") );
