...
my ($window, $titulo, $lblsenha, $ultima, $lblultima) = map { $gladexml->get_widget($_) }qw(window1 label1 label2 label3 label4);
...
sub setup_widgets {
	...
	$lblsenha->modify_font(Gtk2::Pango::FontDescription->from_string("Courier Bold 200"));
	$lblsenha->modify_fg($lblsenha->state, Gtk2::Gdk::Color->parse("#FF4500"));

	$titulo->modify_font(Gtk2::Pango::FontDescription->from_string("Arial 100"));
	$titulo->modify_fg($titulo->state,Gtk2::Gdk::Color->parse("#ccc"));

	$titulo->set_label('Senha       Guichê');

	$ultima->modify_font(Gtk2::Pango::FontDescription->from_string("Arial 100"));
	$ultima->modify_fg($ultima->state,Gtk2::Gdk::Color->parse("#ccc"));
	$ultima->set_label('  Últimas');

	$lblultima->modify_font(Gtk2::Pango::FontDescription->from_string("Courier Bold 100"));
